...
function initEditData(editData) {
	...
	//图片列表设置
	if(editData['视频列表'].length) {
		//将图片列表项渲染
		$('#r-content1 .video-list-con .main').html(''); //清空容器
		$.each(editData['视频列表'], function(i, e) {
			//链接存在  显示链接图标
			str = '<div class="item"><span>' + (i + 1) + '</span><span>' + e['标题'] + '</span><span><img src="' + e['缩略图'].url + '"/><img src="img/link-icon.png" style="display:inline-block" title=' + e['视频项'].url + '></span>' +
				'<span><img src="img/edit.png" class="editImg"/>&nbsp;&nbsp;<img src="img/del.png" class="delImg"/></span></div>';
			$('#r-content1 .video-list-con .main').append(str);
		})
	}
	//副标题设置
...
$('.addGallerySure').click(function() {
	...
	//判断标题
	if(getByteLen(title) < setContentLimitData['视频列表']['标题'][0] || getByteLen(title) > setContentLimitData['视频列表']['标题'][1]) {
		errorTip('标题输入的字符范围为'+setContentLimitData['视频列表']['标题'][0]+'-'+setContentLimitData['视频列表']['标题'][1]+'之间');
		return
	}
	//当标题正确的时候,判断简介
	if((getByteLen(instroduce.trim()) >= setContentLimitData['视频列表']['简介'][0] && getByteLen(instroduce.trim()) <= setContentLimitData['视频列表']['简介'][1]) || (getByteLen(instroduce.trim()) == 0)) {
		obj['简介'] = instroduce;
	} else {
		errorTip('简介输入的字符范围为'+setContentLimitData['视频列表']['简介'][0]+'-'+setContentLimitData['视频列表']['简介'][1]+'之间或者为空');
		return
	}
	//判断url地址 视频项
	...
	str = '<div class="item"><span></span><span>' + title + '</span><span><img src="' + imgSrc + '"/><img src="img/link-icon.png" style="display:inline-block" title=' + links + '></span>' +
		'<span><img src="img/edit.png" class="editImg"/>&nbsp;&nbsp;<img src="img/del.png" class="delImg"/></span></div>';
	$('#r-content .video-list-con .main').append(str);
	//设置序号的值
	$('#r-content .video-list-con .main .item').eq($('#r-content .video-list-con .main .item').length - 1).find('span').eq(0).text($('#r-content .video-list-con .main .item').length);
	$('.addGallery,#mark').hide();
})
$('.newEditGallerySure').click(function() {
	...
	picList[editIndex - 1]['视频项'] = {
		'url': links,
		'size': $('#newEditSelectUrl').attr('dataSize'),
		'format': 'mp4'
	};
	picList[editIndex - 1]['简介'] = instroduce;
	$('#r-content .video-list-con .main .item').eq(editIndex - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', picList[editIndex - 1]['缩略图'].url).end().end().find('img').eq(1).attr('title', links).show();
	$('.newEditGallery,#mark').hide();
})
//新建页面点击图片列表里的编辑按钮
var editIndex; //编辑哪一行
$('#r-content .video-list-con .main').on('click', '.editImg', function() {
	//显示弹框
	...
	$('.newEditGallery .link .form-box').val(data['视频项'].url).removeClass('error-red').addClass('right-gray');
	$('#newEditpreviewImg').attr('src', data['缩略图'].url);
})
//新建页面点击图片列表里的删除按钮
$('#r-content .video-list-con .main').on('click', '.delImg', function() {
	if(confirm('确定删除这行吗')) {
		picList.splice($(this).parent().parent().find('span').eq(0).text() - 1, 1); //删除行对应的picList存储的值
		$(this).parent().parent().remove();
		if($('#r-content .video-list-con .main .item').length) {
			$.each($('#r-content .video-list-con .main .item'), function(i, e) {
...
$('#subm,#submBack').click(function() {
	...
	//当picList列表大于等于1的时候	
	newBuiltAjax(obj, $(this).attr('id'), 'video');
})
//新建页，返回页点击提交
...
function clearNew() {
	...
	$('#r-content .video-list-con .main').html('');
	//清除右侧悬浮高亮显示去除隐藏右侧浮层
...
function clearEdit() {
	...
	//power
	$('#editSelect').val('');
	$('#r-content1 .video-list-con .main').html('');
	//清除右侧悬浮高亮显示去除隐藏右侧浮层
...
$('.updateGallerySure').click(function() {
	...
	if(showval == 'topic') {
		...
		topicEditData['视频列表'][updateEditId - 1]['标题'] = title;
		topicEditData['视频列表'][updateEditId - 1]['缩略图'] = {'url':imgSrc,'size':$('#updatepreviewImg').attr('datasize')};
		topicEditData['视频列表'][updateEditId - 1]['视频项'] = {
			'url': links,
			'size': $('#editEditSelectUrl').attr('dataSize'),
			'format': 'mp4'
		};
		topicEditData['视频列表'][updateEditId - 1]['简介'] = instroduce;
		//显示链接图标
		$('#r-content1 .video-list-con .main .item').eq(updateEditId - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', topicEditData['视频列表'][updateEditId - 1]['缩略图'].url).end().end().find('img').eq(1).attr('title', links).show();
	} else {
		//如果链接存在，显示链接图标，并且移上去可显示链接地址
		//行编辑完成后,改变存储行所在容器自身的值
		editData['视频列表'][updateEditId - 1]['标题'] = title;
		editData['视频列表'][updateEditId - 1]['缩略图'] = {'url':imgSrc,'size':$('#updatepreviewImg').attr('datasize')};
		editData['视频列表'][updateEditId - 1]['视频项'] = {
			'url': links,
			'size': $('#editEditSelectUrl').attr('dataSize'),
			'format': 'mp4'
		};
		editData['视频列表'][updateEditId - 1]['简介'] = instroduce;
		//显示链接图标
		$('#r-content1 .video-list-con .main .item').eq(updateEditId - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', editData['视频列表'][updateEditId - 1]['缩略图'].url).end().end().find('img').eq(1).attr('title', links).show();
	}
	$('.updateGallery,#mark').hide();
})
//编辑页面添加图片确定按钮点击事件
$('.editaddGallerySure').click(function() {
	...
	if(showval == 'topic') {
		topicEditData['视频列表'].push(obj);
	} else {
		editData['视频列表'].push(obj);
	}
	var str;
	//链接存在  显示链接图标
	str = '<div class="item"><span></span><span>' + title + '</span><span><img src="' + imgSrc + '"/><img src="img/link-icon.png" style="display:inline-block" title=' + links + '></span>' +
	...
	$('#r-content1 .video-list-con .main').append(str);
	//设置序号的值
	$('#r-content1 .video-list-con .main .item').eq($('#r-content1 .video-list-con .main .item').length - 1).find('span').eq(0).text($('#r-content1 .video-list-con .main .item').length);
	$('.editaddGallery,#mark').hide();

})
//编辑页面点击图片列表里的编辑按钮
$('#r-content1 .video-list-con .main').on('click', '.editImg', function() {
	//显示弹框
	updateEditId = $(this).parent().parent().find('span').eq(0).text();
	var data;
	if(showval == 'topic') {
		data = topicEditData['视频列表'][updateEditId - 1];
		//设置imgObj值
		imgObj = topicEditData['视频列表'][updateEditId - 1]['缩略图'];
	} else {
		data = editData['视频列表'][updateEditId - 1];
		//设置imgObj值
		imgObj = editData['视频列表'][updateEditId - 1]['缩略图'];
	}
	...
	$('.updateGallery .link .form-box').val(data['视频项'].url).removeClass('error-red').addClass('right-gray');
	$('#updatepreviewImg').attr('src', data['缩略图'].url);
})
//点击图片列表里的删除按钮
$('#r-content1 .video-list-con .main').on('click', '.delImg', function() {
	if(confirm('确定删除这行吗')) {
		if(showval == 'topic') {
			topicEditData['视频列表'].splice($(this).parent().parent().find('span').eq(0).text() - 1, 1); //删除行对应的picList存储的值
		} else {
			editData['视频列表'].splice($(this).parent().parent().find('span').eq(0).text() - 1, 1); //删除行对应的picList存储的值
		}
		$(this).parent().parent().remove();
		if($('#r-content1 .video-list-con .main .item').length) {
			$.each($('#r-content1 .video-list-con .main .item'), function(i, e) {
				$(this).find('span').eq(0).text(i + 1);
			});
		}
	}
...
$('#save').click(function() {
	...
	//图片列表设置
	if(showval == 'topic') {
		if(!topicEditData['视频列表'].length) {
			errorTip('视频列表请添加视频');
			return
		} else {
			//赋值前对picList进行处理，简介或者链接为空的话，删除这个属性
			$.each(topicEditData['视频列表'], function(i, e) {
				for(var key in e) {
					if((key == '简介') && (e[key] == '')) {
						delete e[key];
					}
