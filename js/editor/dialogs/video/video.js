...
(function(){
    ...
    function insertSearch(id){
        ...
        editor.fireEvent('beforeInsertVideo', videoObjs);
    ...
    function insertUpload(){
        ...
        for(var key in uploadVideoList) {
            ...
            videoObjs.push({
                ...
                size: uploadDir + file.size,
                filename: uploadDir + file.filename,
                format: uploadDir + file.format,
                width:width,
                height:height,
                align:align
            });
        ...
        if (count) {
        	...
        	var upload_url = uploadDir + file.url;
    ...
    function insertOnline(){
        ...
        list.forEach(value => {
            videoObjs.push({
                ...
                size: convert_url(value.size),
                format: convert_url(value.format),
                filename: convert_url(value.filename),
                width: 420,
                height: 280,
                align: "none"
            })
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
					...
					filetype = list[i].filename.substr(list[i].filename.lastIndexOf('.') + 1);
                    if ( "png|jpg|jpeg|gif|bmp".indexOf(filetype) != -1 ) {
                        ...
                        textSpan.innerHTML = list[i].filename;
                    ...
                    item.setAttribute('data-size', urlPrefix + list[i].size);
                    item.setAttribute('data-filename', urlPrefix + list[i].filename);
                    if(list[i].filename) {
                    	item.setAttribute('data-format', urlPrefix + list[i].filename.substr(list[i].filename.lastIndexOf('.')+1));
                    }
                    if (list[i].original) {
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    ...
                    var size = lis[i].getAttribute('data-size');
                    var format = lis[i].getAttribute('data-format');
                    var filename = lis[i].getAttribute('data-filename');
                    list.push({
                        title: title,
                        url: url,
                        size: size,
