...
(function(window, undefined) {
	...
	function initPage(id, option) {
		...
		if(isPageNum) {
			...
			pageDiv.style.verticalAlign = 'middle';
			...
			eventUtil.addEvent(select,'change', function(e) {
				...
				if(label) {
					label.innerHTML = '共 <s>' + totalPage + ' </s>页，共 <em>' + totalCount + ' </em>条';
				}
				return false;
			});
		}
		...
		if(isJump) {
			...
			jumpDiv.style.verticalAlign = 'middle';
			jumpDiv.style.display = 'inline-block';
			jumpDiv.style.marginLeft = '5px';
			jumpDiv.style.marginTop = '20px';
			jumpDiv.id = 'jumpDiv';
			button.id = 'jump';
			button.innerHTML = '跳转';
