...
function initEditData(editData) {
	...
	//图片列表设置
	if(editData['图片列表'].length) {
		//将图片列表项渲染
		$('#r-content1 .list-con .main').html(''); //清空容器
		$.each(editData['图片列表'], function(i, e) {
			if(e['链接'] != null) {
				//链接存在  显示链接图标
				str = '<div class="item"><span>' + (i + 1) + '</span><span>' + e['标题'] + '</span><span><img src="' + e['图片项'].url + '"/><img src="img/link-icon.png" style="display:inline-block" title=' + e['链接'] + '></span>' +
					'<span><img src="img/edit.png" class="editImg"/>&nbsp;&nbsp;<img src="img/del.png" class="delImg"/></span></div>';
			} else {
				//链接不存在 隐藏链接图标
				str = '<div class="item"><span>' + (i + 1) + '</span><span>' + e['标题'] + '</span><span><img src="' + e['图片项'].url + '"/><img src="img/link-icon.png" style="display:none" title></span>' +
					'<span><img src="img/edit.png" class="editImg"/>&nbsp;&nbsp;<img src="img/del.png" class="delImg"/></span></div>';
			}
			$('#r-content1 .list-con .main').append(str);
		})
	}
	//副标题设置
...
$('.addGallerySure').click(function() {
	...
	var linkway = $('.addGallery .linkWay select.form-box').val();
	...
	obj['标题'] = title;
	obj['链接打开方式'] = linkway;
	obj['图片项'] = {'url':imgSrc,'size':$('#addpreviewImg').attr('datasize')};
	//添加成功之前进行验证，验证提交的数据是否符合
	//判断标题
	if(getByteLen(title) < setContentLimitData['图片列表']['标题'][0] || getByteLen(title) > setContentLimitData['图片列表']['标题'][1]) {
		errorTip('标题输入的字符范围为'+setContentLimitData['图片列表']['标题'][0]+'-'+setContentLimitData['图片列表']['标题'][1]+'之间');
		return
	}
	//当标题正确的时候,判断简介
	if((getByteLen(instroduce.trim()) >= setContentLimitData['图片列表']['简介'][0] && getByteLen(instroduce.trim()) <= setContentLimitData['图片列表']['简介'][1]) || (getByteLen(instroduce.trim()) == 0)) {
		obj['简介'] = instroduce;
	} else {
		errorTip('简介输入的字符范围为'+setContentLimitData['图片列表']['简介'][0]+'-'+setContentLimitData['图片列表']['简介'][1]+'之间或者为空');
		return
	}
	//当简介正确的时候,判断链接
	if(getByteLen(links.trim()) >= 8 || getByteLen(links.trim()) == 0) {
		obj['链接'] = links;
		linkExist = true;
		if(getByteLen(links.trim()) == 0) {
			linkExist = false;
		}
	} else {
		linkExist = false;
		errorTip('链接输入的字符范围大于8或者为空');
	}
	...
	if(linkExist) {
		...
		//链接不存在 隐藏链接图标
		str = '<div class="item"><span></span><span>' + title + '</span><span><img src="' + imgSrc + '"/><img src="img/link-icon.png" style="display:none" title=' + links + '></span>' +
			'<span><img src="img/edit.png" class="editImg"/>&nbsp;&nbsp;<img src="img/del.png" class="delImg"/></span></div>';
	}
	$('#r-content .list-con .main').append(str);
	//设置序号的值
	$('#r-content .list-con .main .item').eq($('#r-content .list-con .main .item').length - 1).find('span').eq(0).text($('#r-content .list-con .main .item').length);
	$('.addGallery,#mark').hide();
})
$('.newEditGallerySure').click(function() {
	...
	var linkway = $('.newEditGallery .linkWay select.form-box').val();
	...
	//当标题正确的时候,判断简介
	if((getByteLen(instroduce.trim()) >= setContentLimitData['图片列表']['简介'][0] && getByteLen(instroduce.trim()) <= setContentLimitData['图片列表']['简介'][1]) || (getByteLen(instroduce.trim()) == 0)) {} else {
		errorTip('简介输入的字符范围为'+setContentLimitData['图片列表']['简介'][0]+'-'+setContentLimitData['图片列表']['简介'][1]+'之间或者为空');
		return
	}
	//当简介正确的时候,判断链接
	...
	picList[editIndex - 1]['链接打开方式'] = linkway;
	picList[editIndex - 1]['图片项'] = {'url':imgSrc,'size':$('#newEditpreviewImg').attr('datasize')};
	picList[editIndex - 1]['链接'] = links;
	picList[editIndex - 1]['简介'] = instroduce;
	//存在链接这个字段
	if(links.trim() != '') {
		//显示链接图标
		$('#r-content .list-con .main .item').eq(editIndex - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', picList[editIndex - 1]['图片项'].url).end().end().find('img').eq(1).attr('title', links).show();
	} else {
		//隐藏链接图标
		$('#r-content .list-con .main .item').eq(editIndex - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', picList[editIndex - 1]['图片项'].url).end().end().find('img').eq(1).attr('title', links).hide();
	}
	$('.newEditGallery,#mark').hide();
...
//新建页面点击图片列表里的编辑按钮
var editIndex; //编辑哪一行
$('#r-content .list-con .main').on('click', '.editImg', function(e) {
	//显示弹框
	...
	$('.newEditGallery .link .form-box').val(data['链接']).removeClass('error-red').addClass('right-gray');
	$('.newEditGallery .linkWay select.form-box').val(data['链接打开方式']);
	$('#newEditpreviewImg').attr('src', data['图片项'].url);
	e.stopPropagation();
})
//新建页面点击图片列表里的删除按钮
$('#r-content .list-con .main').on('click', '.delImg', function() {
	if(confirm('确定删除这行吗')) {
		picList.splice($(this).parent().parent().find('span').eq(0).text() - 1, 1); //删除行对应的picList存储的值
		$(this).parent().parent().remove();
		if($('#r-content .list-con .main .item').length) {
			$.each($('#r-content .list-con .main .item'), function(i, e) {
...
$('#subm,#submBack').click(function() {
	...
	//当picList列表大于等于1的时候
	newBuiltAjax(obj, $(this).attr('id'), 'gallery');
})
//点击按钮
...
$('.updateGallerySure').click(function() {
	...
	//行编辑完成后,改变存储行所在容器自身的值
	if(showval == 'topic') {
		topicEditData['图片列表'][updateEditId - 1]['标题'] = title;
		topicEditData['图片列表'][updateEditId - 1]['链接打开方式'] = linkway;
		topicEditData['图片列表'][updateEditId - 1]['图片项'] = {'url':imgSrc,'size':$('#updatepreviewImg').attr('datasize')};
		topicEditData['图片列表'][updateEditId - 1]['链接'] = links;
		topicEditData['图片列表'][updateEditId - 1]['简介'] = instroduce;
		if(links.trim() != '') {
			//显示链接图标
			$('#r-content1 .list-con .main .item').eq(updateEditId - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', topicEditData['图片列表'][updateEditId - 1]['图片项'].url).end().end().find('img').eq(1).attr('title', links).show();
		} else {
			//隐藏链接图标
			$('#r-content1 .list-con .main .item').eq(updateEditId - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', topicEditData['图片列表'][updateEditId - 1]['图片项'].url).end().end().find('img').eq(1).attr('title', links).hide();
		}
	} else {
		editData['图片列表'][updateEditId - 1]['标题'] = title;
		editData['图片列表'][updateEditId - 1]['链接打开方式'] = linkway;
		editData['图片列表'][updateEditId - 1]['图片项'] = {'url':imgSrc,'size':$('#updatepreviewImg').attr('datasize')};
		editData['图片列表'][updateEditId - 1]['链接'] = links;
		editData['图片列表'][updateEditId - 1]['简介'] = instroduce;
		if(links.trim() != '') {
			//显示链接图标
			$('#r-content1 .list-con .main .item').eq(updateEditId - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', editData['图片列表'][updateEditId - 1]['图片项'].url).end().end().find('img').eq(1).attr('title', links).show();
		} else {
			//隐藏链接图标
			$('#r-content1 .list-con .main .item').eq(updateEditId - 1).find('span').eq(1).text(title).end().end().find('span').eq(2).find('img').eq(0).attr('src', editData['图片列表'][updateEditId - 1]['图片项'].url).end().end().find('img').eq(1).attr('title', links).hide();
		}
	}
...
$('.editaddGallerySure').click(function() {
	...
	obj['标题'] = title;
	obj['链接打开方式'] = linkway;
	obj['图片项'] = {'url':imgSrc,'size':$('#editaddpreviewImg').attr('datasize')};
	//添加成功之前进行验证，验证提交的数据是否符合
	//判断标题
	if(getByteLen(title) < setContentLimitData['图片列表']['标题'][0] || getByteLen(title) > setContentLimitData['图片列表']['标题'][1]) {
		errorTip('标题输入的字符范围为'+setContentLimitData['图片列表']['标题'][0]+'-'+setContentLimitData['图片列表']['标题'][1]+'之间');
	...
	if(showval == 'topic') {
		topicEditData['图片列表'].push(obj);
	} else {
		editData['图片列表'].push(obj);
	}
	//如果链接存在，显示链接图标，并且移上去可显示链接地址
	if(linkExist) {
		//链接存在  显示链接图标
	...
	$('#r-content1 .list-con .main').append(str);
	//设置序号的值
	$('#r-content1 .list-con .main .item').eq($('#r-content1 .list-con .main .item').length - 1).find('span').eq(0).text($('#r-content1 .list-con .main .item').length);
	$('.editaddGallery,#mark').hide();
})
//编辑页面点击图片列表里的编辑按钮
$('#r-content1 .list-con .main').on('click', '.editImg', function(e) {
	//显示弹框
	updateEditId = $(this).parent().parent().find('span').eq(0).text();
	var data;
	if(showval == 'topic') {
		data = topicEditData['图片列表'][updateEditId - 1];
		//设置imgObj值
		imgObj = topicEditData['图片列表'][updateEditId - 1]['图片项'];
	} else {
		data = editData['图片列表'][updateEditId - 1];
		//设置imgObj值
		imgObj = editData['图片列表'][updateEditId - 1]['图片项'];
	}
	...
	$('.updateGallery .link .form-box').val(data['链接']).removeClass('error-red').addClass('right-gray');
	$('.updateGallery .linkWay select.form-box').val(data['链接打开方式']);
	$('#updatepreviewImg').attr('src', data['图片项'].url);
	e.stopPropagation();
})
//点击图片列表里的删除按钮
$('#r-content1 .list-con .main').on('click', '.delImg', function() {
	if(confirm('确定删除这行吗')) {
		if(showval == 'topic') {
			topicEditData['图片列表'].splice($(this).parent().parent().find('span').eq(0).text() - 1, 1); //删除行对应的picList存储的值
		} else {
			editData['图片列表'].splice($(this).parent().parent().find('span').eq(0).text() - 1, 1); //删除行对应的picList存储的值
		}
		$(this).parent().parent().remove();
		if($('#r-content1 .list-con .main .item').length) {
			$.each($('#r-content1 .list-con .main .item'), function(i, e) {
				$(this).find('span').eq(0).text(i + 1);
			});
		}
	}
...
$('#save').click(function() {
	...
	//图片列表设置
	if(showval == 'topic') {
		if(!topicEditData['图片列表'].length) {
			errorTip('图片列表请添加图片');
			return
		} else {
			//赋值前对picList进行处理，简介或者链接为空的话，删除这个属性
			$.each(topicEditData['图片列表'], function(i, e) {
				for(var key in e) {
					if((key == '简介' || key == '链接') && (e[key] == '')) {
						delete e[key];
					}
